...
class Correlativo_model extends CI_Model
{
    ...
    const correlativos2  =  'pos_correlativos2';
    ...
    function insert_api($correlativos)
    {
        $this->db->truncate(self::correlativos2);

        $data = [];
        foreach ($correlativos as $key => $correlativo) {
            $data[] = $correlativo;
        }
        $this->db->insert_batch(self::correlativos2, $data);
    }
}
