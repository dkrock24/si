...
class Existencias_model extends CI_Model {
    ...
    function get_producto_completo($producto_id){
        ...
        $query = $this->db->query("select p.id_entidad,p.codigo_barras,p.name_entidad,b.nombre_bodega,s.nombre_sucursal , m.moneda_simbolo , pb.Bodega, pb.Cantidad , pd.Utilidad , pd.presentacion , pd.factor, pd.precio, pd.unidad, pd.cod_barra, p.costo from producto as p
                            ...
                            where p.id_entidad = ". $producto_id . " and pd.estado_producto_detalle=1 ORDER BY b.nombre_bodega desc");
